...
ff.roll({configureDialog: false, createMessage: false}).then(chat_message => { if (chat_message) {
    u.chatMessage({ img: i.img, content: i.name }, { small: `On a successful damage dealing attack, Taylor may mark an enemy for one minute (or until she loses concentration), and deal an extra ${damage} damage to it.` })
